...
- **`sglang generate`** â€” run a one-off generation without a persistent server
- **`sglang serve`** â€” launch the OpenAI-compatible HTTP server
...
- `--model-path` â€” which model to load (e.g. `Wan-AI/Wan2.1-T2V-1.3B-Diffusers`)
- `--port` â€” HTTP port to listen on (default: `30010`)
...
| `--tp-size SIZE` | Tensor parallelism size (encoder only; keep â‰¤ 1 when text encoder offload is enabled) |
...
For example, 5 original frames with `exp=1` â†’ 4 gaps Ã— 1 new frame + 5 originals = **9 frames**; with `exp=2` â†’ **17 frames**.

| Argument | Description |
|:--|:--|
| `--enable-frame-interpolation` | Enable frame interpolation. Model weights are downloaded automatically on first use |
| `--frame-interpolation-exp EXP` | Interpolation exponent â€” `1` = 2Ã— temporal resolution, `2` = 4Ã—, etc. (default: `1`) |
...
**Example** â€” generate a 5-frame video and interpolate to 9 frames ($(5 - 1) \times 2^1 + 1 = 9$):
...
Replace the default VAE with a distilled tiny autoencoder for ~3Ã— faster decoding:
...
Users on the diffusers backend can leverage Cache-DiT acceleration by loading custom cache configs from a YAML file. See the [Cache-DiT documentation](/docs/sglang-diffusion/cache-dit) for details.
...
The workflow is: **Generate â†’ Upload â†’ Delete local file**. The API response returns the public URL of the uploaded object.
...
See the [environment variables reference](/docs/sglang-diffusion/environment-variables) for all storage-related variables.
