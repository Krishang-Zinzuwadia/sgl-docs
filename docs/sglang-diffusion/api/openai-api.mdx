...
- `--model-path` â€” path to the model or HuggingFace model ID
- `--port` â€” HTTP port to listen on (default: `30000`)
...
<Steps>
  <Step title="Set LoRA A">
    ```bash
    curl -X POST http://localhost:30010/v1/set_lora \
      -d '{"lora_nickname": "lora_a", "lora_path": "path/to/A"}'
    ```
  </Step>
  <Step title="Generate with LoRA A">
    Run your image or video generation requests.
  </Step>
  <Step title="Unmerge LoRA A">
    ```bash
    curl -X POST http://localhost:30010/v1/unmerge_lora_weights
    ```
  </Step>
  <Step title="Set LoRA B">
    ```bash
    curl -X POST http://localhost:30010/v1/set_lora \
      -d '{"lora_nickname": "lora_b", "lora_path": "path/to/B"}'
    ```
  </Step>
  <Step title="Generate with LoRA B">
...
| `output-compression` | integer | Direct compression level override (0â€“100). When provided, takes precedence over `output-quality` |
